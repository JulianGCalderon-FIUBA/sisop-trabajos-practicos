...
int get_inode_id(superblock_t *superblock, char *path) {
	return _get_inode_id(superblock, superblock->inode_table->inodes[0], path + 1);
}
...
int _get_inode_id(superblock_t *superblock, inode_t dir, char *path) {
	...
	while ((dir_entry = read_directory(dir, offset))) {
		...
		if (end_of_path || (!(slash_pos + 1) && subdir->type == DIR)) // end of path, or path had trailing '/' and found a dir
...
int init_inodes(superblock_t *superblock) {
	superblock->tables = {0}; // initialise all pointers to 0
