...
read_directory(inode_t *dir, size_t offset, dir_entry_t *dir_entry_dest)
{
	...
	if (ret_val != sizeof(dir_entry_t)) {
		...
		return ret_val >= 0
		               ? -1
		               : -ret_val;  // if inode_read didn't return an error, return -1
	}
	return EXIT_SUCCESS;
}
...
static void
init_link(dir_entry_t *dir_entry, int target_inode_id, const char *target_name)
{
 ...
 */
int
create_dir(superblock_t *superblock, const char *name, int parent_inode_id)
{
